...
// ---------- NEW: English Tutor endpoint (SAFE ADDITION) ----------
app.post("/api/tutor", async (req, res) => {
  try {
    const { message, meta = {} } = req.body || {};

    if (!message) {
      return res.json({ reply: "Please say something so we can practice English." });
    }

    const prompt = `
You are a friendly English tutor for Indian learners.
Speak slowly and clearly.
Use simple English words.

Level: ${meta.level || "Beginner"}
Native language for help: ${meta.nativeLanguage || "English only"}
Topic: ${meta.subject || "Daily conversation"}

Rules:
- Encourage the learner.
- Correct mistakes gently.
- If needed, explain briefly in the native language.
...
`;
    ...
    const replyText = await callOpenRouterText({
    ...
    res.status(500).json({ reply: "Sorry, the tutor is not available right now." });
  }
});
